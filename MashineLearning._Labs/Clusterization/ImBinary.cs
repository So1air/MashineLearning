...
namespace MashineLearning.Clusterization
{
    ...
    class ImBinary
    {
        ...
        public int DistanceTo(ImBinary otherIm)  //манхеттенська відстань
        {
            if ((otherIm == null) || (otherIm._row_count != this._row_count) || (otherIm._col_count != this._col_count))
                return -1;

            int distance = 0;
            int i = this._properties.Length;
            while (i-- != 0)
                if (otherIm._properties[i] != this._properties[i]) 
                    distance++;
            return distance;            
        }
